...
input:
  ...
  Change the M_scat2 definition if your final nucleon is not the same as the initial one! Set it to the final nucleon mass!
...
int fmotion(double rndm_t,double rndm_phi,int ipn,int ims,double q,double nu,double W2, double Pp,
            double thetap,double phip,double thetag,double phig,double ftmin, double ftmax,double *out)
{
  double M_TARG2(m_targ(ipn,2.));
  double M_scat2 = M_TARG2;          // Change this here if it's a charge-exchange process and your final nucleon is not the same as the initial one!!
  ...
  if (ims == -1) qp0  = (W2 - M_scat2)/(2.*W);
  else qp0  = sqrt(0.25 * sqr(W2 - M_scat2 - m_ms(ims,2)) - M_scat2 * m_ms(ims,2))/W;
  if(qp0 < 0.)
    {
  ...
  // Calculate virtual photon angles with CM in the Lab frame
  ...
  nu0 = gacm*(nu - btcm*ql);
  if(nu0<0)    // Virtual photon! It may well be negative in CM since in even in Lab q > nu. 
    {
  ...
  if(sthg0 != 0.0)  // transverse components unaffected by boost
  ...
  else
    {
      ...
      sphig0 = 0.0;   // but actually why not keep the original phi? Since it doesn't matter...
    }
    
 // Now you have the virtual photon four-momentum in the CM frame, w.r.t. axes defined by Pcm
...
// Polar angle of the incident nucleon with CM direction
  ...
  if(abs(cthpfcm) > 1.) cthpfcm = cthpfcm/abs(cthpfcm);
...
// ims 0 = pi0, 1 = eta. -1 is the value passed in place of ims if it's a real photon
  ...
  // Make sure that the t-range is defined by the more restrictive values out of the calculated min and max t and those set by the user:

  //cout << "calculated tmin, tmax: " << tmin << ", " << tmax << endl;
  //cout << "User range: " << ftmin << ", " << ftmax << endl; 
  
  tmin   = max(tmin, ftmin);
  tmax   = min(tmax, ftmax);

  if (tmin > tmax){
    //cout << "Calculation of tmin and tmax gives values outside of range selected by user, bad event!" << endl;
    return -1;
  }
  ...
  xabs = fabs(cthqqp0);
  if(xabs > 1.)    // sometimes it's right on the edge -- but it should be a physical value regardless, from the definition of t
    {
      // cout<<"bad generated t, cos(thqqp0): "<< cthqqp0 << endl;
      // return -1;
      cthqqp0 = cthqqp0/fabs(cthqqp0);
    }
...
// Polar angle
  ...
  if(abs(cthqp0) > 1.) cthqp0 = cthqp0/abs(cthqp0);
...
// produced particle and virtual photon relative angle to check t
  ...
  // if(abs(ttmp-t)>tiny())
  if(abs(ttmp-t)>0.001)    // they should agree else something has gone wrong!
    {
    cout<<"t check "<<endl;
...
// Calculate the momentum and angle of the recoil nucleon
  ...
  Epp = sqrt(M_scat2 + Ppp*Ppp);   // in Lab
